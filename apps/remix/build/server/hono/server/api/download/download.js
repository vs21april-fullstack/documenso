import { PDF_SIZE_A4_72PPI } from '../../../packages/lib/constants/pdf.js';
import { AppError, AppErrorCode } from '../../../packages/lib/errors/app-error.js';
import { getEnvelopeById, getEnvelopeWhereInput } from '../../../packages/lib/server-only/envelope/get-envelope-by-id.js';
import { generateAuditLogPdf } from '../../../packages/lib/server-only/pdf/generate-audit-log-pdf.js';
import { generateCertificatePdf } from '../../../packages/lib/server-only/pdf/generate-certificate-pdf.js';
import { getApiTokenByToken } from '../../../packages/lib/server-only/public-api/get-api-token-by-token.js';
import { isDocumentCompleted } from '../../../packages/lib/utils/document.js';
import { buildTeamWhereQuery } from '../../../packages/lib/utils/teams.js';
...
import { handleEnvelopeItemFileRequest } from '../files/files.helpers.js';
import { ZDownloadEnvelopeItemRequestParamsSchema, ZDownloadEnvelopeItemRequestQuerySchema, ZDownloadEnvelopeAuditLogPdfRequestParamsSchema, ZDownloadEnvelopeCertificatePdfRequestParamsSchema, ZDownloadDocumentRequestParamsSchema } from './download.types.js';
 ...
 */
const resolveApiToken = async authorizationHeader => {
 ...
 */.get('/envelope/item/:envelopeItemId/download', sValidator('param', ZDownloadEnvelopeItemRequestParamsSchema), sValidator('query', ZDownloadEnvelopeItemRequestQuerySchema), async c => {
 ...
 * Requires API key authentication via Authorization header.
 */.get('/envelope/:envelopeId/audit-log/download', sValidator('param', ZDownloadEnvelopeAuditLogPdfRequestParamsSchema), async c => {
  const logger = c.get('logger');
  try {
    const {
      envelopeId
 ...
 * Requires API key authentication via Authorization header.
 */.get('/envelope/:envelopeId/certificate/download', sValidator('param', ZDownloadEnvelopeCertificatePdfRequestParamsSchema), async c => {
  const logger = c.get('logger');
  try {
    const {
      envelopeId
