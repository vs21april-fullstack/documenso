...
const buildCspHeader = ({
  ...
  // is present.
  const directives = [`base-uri 'self'`, `object-src 'none'`, `form-action 'self'`, `script-src 'self' 'nonce-${nonce}' ${PUBLISHED_APP_BOOTSTRAP_HASH} 'strict-dynamic'`,
 ...
 */
const securityHeadersMiddleware = createMiddleware(async (c, next) => {
  ...
  c.res.headers.set('Content-Security-Policy', buildCspHeader({
    nonce,
    kind
  }));
...
export { CSP_NONCE_KEY, securityHeadersMiddleware };
