 ...
 */
const ZLicenseClaimSchema = z.object({
 ...
 * Note: Keep this in sync with the Documenso License Server schemas.
 */
const ZLicenseResponseSchema = z.object({
  success: z.boolean(),
 ...
 */
const ZCachedLicenseSchema = z.object({
  /**
   ...
   */
  derivedStatus: z.enum(['UNAUTHORIZED',
  // Unauthorized flag usage detected, overrides everything except PAST_DUE since that's a grace period.
  'ACTIVE',
  // License is active and everything is good.
  'EXPIRED',
  // License is expired and there is no unauthorized flag usage.
  'PAST_DUE',
  // License is past due.
...
export { LICENSE_FILE_NAME, ZCachedLicenseSchema, ZLicenseClaimSchema, ZLicenseResponseSchema };
