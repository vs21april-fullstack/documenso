...
class BullMQJobProvider extends BaseJobProvider {
  _jobDefinitions = {};
  constructor() {
    super();
    ...
    this._worker = new Worker(QUEUE_NAME, async job => {
  ...
  defineJob(definition) {
    ...
    if (definition.trigger.cron && definition.enabled !== false) {
      void this._queue.upsertJobScheduler(definition.id, {
  ...
  async triggerJob(options) {
    ...
    await Promise.all(eligibleJobs.map(async job => {
      const backgroundJob = await prismaWithReplicas.backgroundJob.create({
        data: {
          jobId: job.id,
          name: job.name,
          version: job.version,
  ...
  getApiHandler() {
    ...
    return async c => {
      ...
      if (env('NODE_ENV') !== 'development') {
        const {
          getOptionalSession
        } = await import('../../../auth/server/lib/utils/get-session.js');
        const {
          isAdmin
        } = await import('../../utils/is-admin.js');
        const {
          user
  ...
  async processJob(job) {
    ...
    if (backgroundJobId) {
      await prismaWithReplicas.backgroundJob.update({
        where: {
          id: backgroundJobId,
    ...
    return {
      runTask: async (cacheKey, callback) => {
        ...
        try {
          ...
          await prismaWithReplicas.backgroundJobTask.update({
            where: {
              id: task.id,
              jobId
            },
