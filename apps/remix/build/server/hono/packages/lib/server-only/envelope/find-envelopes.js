...
const findEnvelopes = async ({
  ...
  const visibilityFilter = eb => eb.or([eb('Envelope.visibility', 'in', allowedVisibilities.map(v => sql.lit(v))),
  ...
  const maskedData = data.map(envelope => maskRecipientTokensForDocument({
    document: envelope,
    user
  }));
