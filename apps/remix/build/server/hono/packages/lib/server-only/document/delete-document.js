...
import { EnvelopeType, WebhookTriggerEvents, DocumentStatus, SendStatus, RecipientRole } from '@prisma/client';
...
const handleDocumentOwnerDelete = async ({
  ...
  const deletedEnvelope = await prismaWithReplicas.$transaction(async tx => {
  ...
  const recipientsToNotify = envelope.recipients.filter(recipient => recipient.sendStatus === SendStatus.SENT && recipient.role !== RecipientRole.CC && isRecipientEmailValidForSending(recipient)).map(recipient => ({
