...
import { convertPlaceholdersToFieldInputs } from '../pdf/auto-place-fields.js';
...
import { createDocumentAuthOptions, createRecipientAuthOptions } from '../../utils/document-auth.js';
...
const createEnvelope = async ({
  ...
  if (normalizePdf$1) {
    envelopeItems = await Promise.all(data.envelopeItems.map(async item => {
      const documentData = await prismaWithReplicas.documentData.findFirst({
        where: {
      ...
      const normalizedPdf = await normalizePdf(Buffer.from(buffer), {
  ...
  const recipientsHaveActionAuth = data.recipients?.some(recipient => recipient.actionAuth && recipient.actionAuth.length > 0);
  // Check if user has permission to set the global action auth.
  if ((authOptions.globalActionAuth.length > 0 || recipientsHaveActionAuth) && !team.organisation.organisationClaim.flags.cfr21) {
  ...
  const getValidatedDelegatedOwner = async () => {
    ...
    const delegatedOwner = await prismaWithReplicas.user.findFirst({
      where: {
  ...
  const [documentMeta, secondaryId, delegatedOwner] = await Promise.all([prismaWithReplicas.documentMeta.create({
    data: extractDerivedDocumentMeta(settings, {
      ...meta,
      timezone: timezoneToUse
    }, signatureLevel)
  }), type === EnvelopeType.DOCUMENT ? incrementDocumentId().then(v => v.formattedDocumentId) : incrementTemplateId().then(v => v.formattedTemplateId), getValidatedDelegatedOwner()]);
  const envelopeOwnerId = delegatedOwner?.id ?? userId;
  const createdEnvelope = await prismaWithReplicas.$transaction(async tx => {
    ...
    const defaultRecipients = settings.defaultRecipients && !bypassDefaultRecipients ? ZDefaultRecipientsSchema.parse(settings.defaultRecipients) : [];
    ...
    await Promise.all(allRecipients.map(async recipient => {
      ...
      const recipientFieldsToCreate = (recipient.fields || []).map(field => {
        ...
        if (field.documentDataId) {
          const foundEnvelopeItem = envelope.envelopeItems.find(item => item.documentDataId === field.documentDataId);
    ...
    if (itemsWithPlaceholders.length > 0) {
      ...
      const uniqueRecipientRefs = new Map();
      ...
      const shouldCreatePlaceholderRecipients = (!data.recipients || data.recipients.length === 0) && uniqueRecipientRefs.size > 0;
      ...
      for (const item of itemsWithPlaceholders) {
        ...
        const fieldsToCreate = convertPlaceholdersToFieldInputs(item.placeholders ?? [], (recipientPlaceholder, placeholder) => findRecipientByPlaceholder(recipientPlaceholder, placeholder, data.recipients && data.recipients.length > 0 ? data.recipients.map(r => {
          ...
          return found;
        }) : undefined, availableRecipients), envelopeItem.id);
