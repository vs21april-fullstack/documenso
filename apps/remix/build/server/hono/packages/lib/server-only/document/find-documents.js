...
import { EnvelopeType, TeamMemberRole, DocumentVisibility, DocumentStatus, SigningStatus, RecipientRole } from '@prisma/client';
import { DateTime } from 'luxon';
import { match } from 'ts-pattern';
import { maskRecipientTokensForDocument } from '../../utils/mask-recipient-tokens-for-document.js';
...
const findDocuments = async ({
  ...
  const buildBaseQuery = () => {
    ...
    if (hasSearch) {
      qb = qb.where(({
        or,
        eb
      }) => or([eb('Envelope.title', 'like', searchPattern), eb('Envelope.externalId', 'like', searchPattern),
      // Capped recipient search subquery (uses trigram indexes)
      eb('Envelope.id', 'in', eb.selectFrom('Recipient').select('Recipient.envelopeId').where(({
        or: innerOr,
        eb: innerEb
      }) => innerOr([innerEb('Recipient.email', 'like', searchPattern), innerEb('Recipient.name', 'like', searchPattern)])).distinct().limit(RECIPIENT_SEARCH_CAP))]));
    }
    return qb;
  };
  // ─── Personal path filters ───────────────────────────────────────────
  const applyPersonalFilters = qb => {
    // Deleted filter: owned → deletedAt IS NULL, received → documentDeletedAt IS NULL
    const personalDeletedFilter = eb => eb.or([eb.and([eb('Envelope.userId', '=', user.id), eb('Envelope.deletedAt', 'is', null)]), recipientExists(eb, user.email, reb => reb('Recipient.documentDeletedAt', 'is', null))]);
    return match(status).with(ExtendedDocumentStatus.ALL, () => qb.where(eb => eb.and([personalDeletedFilter(eb), eb.or([eb('Envelope.userId', '=', user.id), eb.and([eb('Envelope.status', 'in', [sql.lit(DocumentStatus.COMPLETED), sql.lit(DocumentStatus.PENDING), sql.lit(DocumentStatus.CANCELLED)]), recipientExists(eb, user.email)])])]))).with(ExtendedDocumentStatus.INBOX, () => qb.where('Envelope.status', '!=', sql.lit(ExtendedDocumentStatus.DRAFT)).where(eb =>
    ...
    recipientExists(eb, user.email, reb => reb.and([reb('Recipient.documentDeletedAt', 'is', null), reb('signingStatus', '=', sql.lit(SigningStatus.NOT_SIGNED)), reb('role', '!=', sql.lit(RecipientRole.CC))])))).with(ExtendedDocumentStatus.DRAFT, () => qb.where('Envelope.userId', '=', user.id).where('Envelope.deletedAt', 'is', null).where('Envelope.status', '=', sql.lit(DocumentStatus.DRAFT))).with(ExtendedDocumentStatus.PENDING, () => qb.where('Envelope.status', '=', sql.lit(DocumentStatus.PENDING)).where(eb => eb.and([personalDeletedFilter(eb), eb.or([eb('Envelope.userId', '=', user.id), recipientExists(eb, user.email, reb => reb.and([reb('Recipient.signingStatus', '=', sql.lit(SigningStatus.SIGNED)), reb('Recipient.role', '!=', sql.lit(RecipientRole.CC))]))])]))).with(ExtendedDocumentStatus.COMPLETED, () => qb.where('Envelope.status', '=', sql.lit(ExtendedDocumentStatus.COMPLETED)).where(eb => eb.and([personalDeletedFilter(eb), eb.or([eb('Envelope.userId', '=', user.id), recipientExists(eb, user.email)])]))).with(ExtendedDocumentStatus.REJECTED, () => qb.where('Envelope.status', '=', sql.lit(ExtendedDocumentStatus.REJECTED)).where(eb => eb.and([personalDeletedFilter(eb), eb.or([eb('Envelope.userId', '=', user.id), recipientExists(eb, user.email, reb => reb('Recipient.signingStatus', '=', sql.lit(SigningStatus.REJECTED)))])]))).with(ExtendedDocumentStatus.CANCELLED, () => qb.where('Envelope.status', '=', sql.lit(ExtendedDocumentStatus.CANCELLED)).where(eb => eb.and([personalDeletedFilter(eb), eb.or([eb('Envelope.userId', '=', user.id), recipientExists(eb, user.email)])]))).exhaustive();
  };
  // ─── Team path filters ───────────────────────────────────────────────
  const applyTeamFilters = (qb, teamData) => {
    ...
    return match(status).with(ExtendedDocumentStatus.ALL, () => qb.where(eb => {
      const accessBranches = [eb('Envelope.teamId', '=', teamData.id)];
      if (teamEmail) {
        accessBranches.push(senderEmailIs(eb, teamEmail));
        accessBranches.push(eb.and([eb('status', '!=', sql.lit(ExtendedDocumentStatus.DRAFT)), recipientExists(eb, teamEmail)]));
      }
      ...
      return qb.where('Envelope.status', '!=', sql.lit(ExtendedDocumentStatus.DRAFT)).where(eb => eb.and([visibilityFilter(eb),
      ...
      recipientExists(eb, teamEmail, reb => reb.and([reb('Recipient.documentDeletedAt', 'is', null), reb('Recipient.signingStatus', '=', sql.lit(SigningStatus.NOT_SIGNED)), reb('Recipient.role', '!=', sql.lit(RecipientRole.CC))]))]));
    }).with(ExtendedDocumentStatus.DRAFT, () => qb.where('Envelope.status', '=', sql.lit(ExtendedDocumentStatus.DRAFT)).where(eb => {
      const accessBranches = [eb('Envelope.teamId', '=', teamData.id)];
      if (teamEmail) {
        accessBranches.push(senderEmailIs(eb, teamEmail));
      }
      return eb.and([teamDeletedFilter(eb), visibilityFilter(eb), eb.or(accessBranches)]);
    })).with(ExtendedDocumentStatus.PENDING, () => qb.where('Envelope.status', '=', sql.lit(ExtendedDocumentStatus.PENDING)).where(eb => {
      const accessBranches = [eb('Envelope.teamId', '=', teamData.id)];
      if (teamEmail) {
        accessBranches.push(senderEmailIs(eb, teamEmail));
      ...
      return eb.and([teamDeletedFilter(eb), visibilityFilter(eb), eb.or(accessBranches)]);
    })).with(ExtendedDocumentStatus.COMPLETED, () => qb.where('Envelope.status', '=', sql.lit(ExtendedDocumentStatus.COMPLETED)).where(eb => {
      const accessBranches = [eb('Envelope.teamId', '=', teamData.id)];
      if (teamEmail) {
        accessBranches.push(senderEmailIs(eb, teamEmail));
      ...
      return eb.and([teamDeletedFilter(eb), visibilityFilter(eb), eb.or(accessBranches)]);
    })).with(ExtendedDocumentStatus.REJECTED, () => qb.where('Envelope.status', '=', sql.lit(ExtendedDocumentStatus.REJECTED)).where(eb => {
      const accessBranches = [eb('Envelope.teamId', '=', teamData.id)];
      if (teamEmail) {
        accessBranches.push(senderEmailIs(eb, teamEmail));
      ...
      return eb.and([teamDeletedFilter(eb), visibilityFilter(eb), eb.or(accessBranches)]);
    })).with(ExtendedDocumentStatus.CANCELLED, () => qb.where('Envelope.status', '=', sql.lit(ExtendedDocumentStatus.CANCELLED)).where(eb => {
      const accessBranches = [eb('Envelope.teamId', '=', teamData.id)];
      if (teamEmail) {
        accessBranches.push(senderEmailIs(eb, teamEmail));
        accessBranches.push(recipientExists(eb, teamEmail));
  ...
  const countQuery = useWindowedCount ? kyselyPrisma.$kysely.selectFrom(baseCountQuery.limit(offset + COUNT_WINDOW_SIZE * perPage + 1).as('windowed')).select(({
    fn
  }) => fn.count('id').as('total')) : kyselyPrisma.$kysely.selectFrom(baseCountQuery.as('filtered')).select(({
    fn
  ...
  const totalCount = useWindowedCount ? Math.min(Number(countResult.total ?? 0), offset + COUNT_WINDOW_SIZE * perPage) : Number(countResult.total ?? 0);
  // ─── Hydrate with Prisma ─────────────────────────────────────────────
  if (ids.length === 0) {
    return {
      data: [],
  ...
  const data = await prismaWithReplicas.envelope.findMany({
    where: {
      id: {
        in: ids
      }
