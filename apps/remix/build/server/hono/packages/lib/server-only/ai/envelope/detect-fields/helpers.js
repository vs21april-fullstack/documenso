 ...
 */
const buildRecipientContextMessage = recipients => {
 ...
 */
const formatRecipientKey = recipient => {
 ...
 */
const resolveRecipientFromKey = (recipientKey, recipients) => {
  if (recipients.length === 0) {
    return null;
  }
 ...
 */
const normalizeDetectedField = field => {
  const {
    box2d
  } = field;
...
export { buildRecipientContextMessage, formatRecipientKey, normalizeDetectedField, resolveRecipientFromKey };
