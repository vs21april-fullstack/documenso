...
import { DocumentSigningOrder, RecipientRole, FieldType, DocumentStatus, DocumentSource, EnvelopeType, SigningStatus, SendStatus, Prisma, WebhookTriggerEvents } from '@prisma/client';
import { DateTime } from 'luxon';
import { match } from 'ts-pattern';
import { z } from 'zod';
import { AppError, AppErrorCode } from '../../errors/app-error.js';
import { jobs } from '../../jobs/client.js';
import { DOCUMENT_AUDIT_LOG_TYPE, RECIPIENT_DIFF_TYPE } from '../../types/document-audit-logs.js';
import { DocumentAccessAuth, ZRecipientAuthOptionsSchema } from '../../types/document-auth.js';
...
import { isRequiredField } from '../../utils/advanced-fields-helpers.js';
...
import { sendDocument } from '../document/send-document.js';
...
const ZCreateDocumentFromDirectTemplateResponseSchema = z.object({
  token: z.string(),
  envelopeId: z.string(),
  documentId: z.number(),
...
const createDocumentFromDirectTemplate = async ({
  ...
  const directTemplateEnvelope = await prismaWithReplicas.envelope.findFirst({
    where: {
      directLink: {
  ...
  if (nextSigner && (!directTemplateEnvelope.documentMeta?.allowDictateNextSigner || directTemplateEnvelope.documentMeta?.signingOrder !== DocumentSigningOrder.SEQUENTIAL)) {
    throw new AppError(AppErrorCode.INVALID_REQUEST, {
  ...
  const isAccessAuthValid = match(derivedRecipientAccessAuth.at(0)).with(DocumentAccessAuth.ACCOUNT, () => user && user?.email === directRecipientEmail).with(DocumentAccessAuth.TWO_FACTOR_AUTH, () => false) // Not supported for direct templates
  .with(undefined, () => true).exhaustive();
  ...
  const nonDirectTemplateRecipients = directTemplateEnvelope.recipients.filter(recipient => recipient.id !== directTemplateRecipient.id);
  ...
  const createDirectRecipientFieldArgs = await Promise.all(fieldsToProcess.map(async templateField => {
    const signedFieldValue = signedFieldValues.find(value => value.fieldId === templateField.id);
    ...
    const isSignatureField = templateField.type === FieldType.SIGNATURE || templateField.type === FieldType.FREE_SIGNATURE;
  ...
  const envelopeItemsToCreate = await Promise.all(directTemplateEnvelope.envelopeItems.map(async (item, i) => {
  ...
  const {
    ...
    Object.values(nonDirectTemplateRecipients).forEach(templateRecipient => {
      ...
      nonDirectRecipientFieldsToCreate = nonDirectRecipientFieldsToCreate.concat(templateRecipient.fields.map(field => ({
    ...
    const createdDirectRecipient = await tx.recipient.create({
      data: {
        ...
        fields: {
          createMany: {
            data: directTemplateNonSignatureFields.map(({
              templateField,
              customText
            }) => {
              let inserted = true;
    ...
    const createdDirectRecipientSignatureFields = await Promise.all(directTemplateSignatureFields.map(async ({
      templateField,
      signature,
      derivedRecipientActionAuth
    }) => {
    ...
    const createdDirectRecipientFields = [...createdDirectRecipient.fields.map(field => ({
      field,
     ...
     */
    const auditLogsToCreate = [createDocumentAuditLogData({
    ...
    }), ...createdDirectRecipientFields.map(({
      field,
      derivedRecipientActionAuth
    }) => createDocumentAuditLogData({
      ...
      data: {
        ...
        field: match(field.type).with(FieldType.SIGNATURE, FieldType.FREE_SIGNATURE, type => ({
          type,
          data: field.signature?.signatureImageAsBase64 || field.signature?.typedSignature || ''
        })).with(FieldType.DATE, FieldType.EMAIL, FieldType.INITIALS, FieldType.NAME, FieldType.TEXT, FieldType.NUMBER, FieldType.CHECKBOX, FieldType.DROPDOWN, FieldType.RADIO, type => ({
          type,
    ...
    if (nextSigner) {
      ...
      if (nextRecipient) {
        auditLogsToCreate.push(createDocumentAuditLogData({
...
export { ZCreateDocumentFromDirectTemplateResponseSchema, createDocumentFromDirectTemplate };
//# sourceMappingURL=create-document-from-direct-template.js.map
