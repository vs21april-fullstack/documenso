...
import { ZFieldMetaSchema, FIELD_META_DEFAULT_VALUES, ZTextFieldMeta, ZNumberFieldMeta, ZCheckboxFieldMeta, ZRadioFieldMeta, ZDropdownFieldMeta } from '../../types/field-meta.js';
...
const setFieldsForDocument = async ({
  userId,
  teamId,
  id,
  fields,
  ...
  const removedFields = existingFields.filter(existingField => !fields.find(field => field.id === existingField.id));
  ...
  const persistedFields = await prismaWithReplicas.$transaction(async tx => {
    return await Promise.all(linkedFields.map(async field => {
      const fieldSignerEmail = field._recipient.email.toLowerCase();
      const parsedFieldMeta = field.fieldMeta ? ZFieldMetaSchema.parse(field.fieldMeta) : FIELD_META_DEFAULT_VALUES[field.type];
      ...
      if (field.type === FieldType.CHECKBOX) {
        if (field.fieldMeta) {
          ...
          const errors = validateCheckboxField(checkboxFieldParsedMeta?.values?.map(item => item.value) ?? [], checkboxFieldParsedMeta);
          if (errors.length > 0) {
            throw new Error(errors.join(', '));
          }
        } else {
  ...
  if (removedFields.length > 0) {
    await prismaWithReplicas.$transaction(async tx => {
      ...
      await tx.documentAuditLog.createMany({
        data: removedFields.map(field => createDocumentAuditLogData({
  ...
  const mappedFilteredFields = existingFields.filter(field => {
...
const hasFieldBeenChanged = (field, newFieldData) => {
  const currentFieldMeta = field.fieldMeta || null;
  const newFieldMeta = newFieldData.fieldMeta || null;
  return field.envelopeItemId !== newFieldData.envelopeItemId || field.type !== newFieldData.type || field.page !== newFieldData.pageNumber || field.positionX.toNumber() !== newFieldData.pageX || field.positionY.toNumber() !== newFieldData.pageY || field.width.toNumber() !== newFieldData.pageWidth || field.height.toNumber() !== newFieldData.pageHeight || !isDeepEqual(currentFieldMeta, newFieldMeta);
};

export { setFieldsForDocument };
//# sourceMappingURL=set-fields-for-document.js.map
