...
import { EnvelopeType, DocumentStatus, SigningStatus, RecipientRole, OrganisationType, SendStatus, WebhookTriggerEvents } from '@prisma/client';
...
import { buildEnvelopeEmailHeaders } from '../email/build-envelope-email-headers.js';
...
import { updateRecipientNextReminder } from '../recipient/update-recipient-next-reminder.js';
import { assertUserNotDisabled } from '../user/assert-user-not-disabled.js';
...
const resendDocument = async ({
  ...
  const recipientsToRemind = envelope.recipients.filter(recipient => recipients.includes(recipient.id) && recipient.signingStatus === SigningStatus.NOT_SIGNED && recipient.role !== RecipientRole.CC);
  ...
  await Promise.all(recipientsToRemind.map(recipient => updateRecipientNextReminder({
  ...
  await Promise.all(recipientsToRemind.map(async recipient => {
    ...
    if (organisationType === OrganisationType.ORGANISATION) {
      ...
      emailMessage = envelope.documentMeta.message || i18n._(
      /*i18n*/
      {
        id: "smtt5l",
        values: {
          0: user.name || user.email,
          1: envelope.team.name,
          2: envelope.title,
          recipientActionVerb: recipientActionVerb
        }
      });
    ...
    const template = /*#__PURE__*/createElement(DocumentInviteEmailTemplate, {
      ...
      inviterEmail: organisationType === OrganisationType.ORGANISATION ? envelope.team?.teamEmail?.email || user.email : user.email,
      assetBaseUrl,
      signDocumentLink,
    ...
    await emailTransport.sendMail({
      ...
      subject: envelope.documentMeta.subject ? renderCustomEmailTemplate(i18n._(
      /*i18n*/
      {
        id: "paGeq9",
        values: {
      ...
      }), customEmailTemplate) : emailSubject,
      html,
      text,
...
export { resendDocument };
//# sourceMappingURL=resend-document.js.map
