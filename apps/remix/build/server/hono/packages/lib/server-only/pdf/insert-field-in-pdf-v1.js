import { RotationTypes, radiansToDegrees, degrees, rgb, TextAlignment, setFontAndSize } from '@cantoo/pdf-lib';
import { DEFAULT_HANDWRITING_FONT_SIZE, DEFAULT_STANDARD_FONT_SIZE, MIN_HANDWRITING_FONT_SIZE, MIN_STANDARD_FONT_SIZE } from '../../constants/pdf.js';
...
import { ZCheckboxFieldMeta, ZRadioFieldMeta, ZInitialsFieldMeta, ZNameFieldMeta, ZEmailFieldMeta, ZDateFieldMeta, ZNumberFieldMeta, ZTextFieldMeta } from '../../types/field-meta.js';
import { getPageSize } from './get-page-size.js';
...
const insertFieldInPDFV1 = async (pdf, field) => {
  ...
  await match(field).with({
    ...
    const selected = fromCheckboxValue(field.customText);
    ...
    const fieldMetaParser = fieldMetaParsers[field.type];
    ...
    if (pageRotationInDegrees !== 0) {
      const adjustedPosition = adjustPositionForRotation(pageWidth, pageHeight, adjustedFieldX, adjustedFieldY, pageRotationInDegrees);
...
export { insertFieldInPDFV1 };
//# sourceMappingURL=insert-field-in-pdf-v1.js.map
