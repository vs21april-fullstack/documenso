import { EnvelopeType, DocumentStatus, RecipientRole, SigningStatus, SendStatus } from '@prisma/client';
 ...
 */
const unsafeBuildEnvelopeIdQuery = (options, expectedEnvelopeType) => {
  return match(options).with({
    type: 'envelopeId'
  }, value => {
 ...
 */
const unsafeBuildEnvelopeIdsQuery = (options, expectedEnvelopeType) => {
 ...
 */
const mapDocumentIdToSecondaryId = documentId => {
 ...
 */
const mapTemplateIdToSecondaryId = templateId => {
 ...
 */
const mapSecondaryIdToDocumentId = secondaryId => {
 ...
 */
const mapSecondaryIdToTemplateId = secondaryId => {
...
const getEnvelopeItemPermissions = (envelope, recipients) => {
  // Always reject completed/rejected/cancelled/deleted envelopes.
  if (envelope.completedAt || envelope.deletedAt || envelope.status === DocumentStatus.REJECTED || envelope.status === DocumentStatus.COMPLETED || envelope.status === DocumentStatus.CANCELLED) {
    return {
  ...
  const hasActiveRecipients = recipients.some(recipient => recipient.role !== RecipientRole.CC && (recipient.signingStatus === SigningStatus.SIGNED || recipient.signingStatus === SigningStatus.REJECTED || recipient.sendStatus === SendStatus.SENT));
  return match(envelope.status).with(DocumentStatus.DRAFT, () => ({
    canTitleBeChanged: true,
    canFileBeChanged: true,
    canOrderBeChanged: true
...
export { getEnvelopeItemPermissions, mapDocumentIdToSecondaryId, mapSecondaryIdToDocumentId, mapSecondaryIdToTemplateId, mapTemplateIdToSecondaryId, unsafeBuildEnvelopeIdQuery, unsafeBuildEnvelopeIdsQuery };
