...
const ZSanitizeBrandingCssWarningSchema = z.object({
  kind: z.enum(['selector', 'property', 'value', 'at-rule', 'parse-error']),
  detail: z.string(),
  line: z.number().optional()
});
const BLOCKED_PROPERTIES = new Set(['display', 'visibility', 'opacity', 'pointer-events', 'position', 'inset', 'top', 'right', 'bottom', 'left', 'z-index', 'transform', 'clip', 'clip-path', 'mask', 'mask-image', 'content', 'width', 'height', 'min-width', 'min-height', 'max-width', 'max-height', 'overflow', 'overflow-x', 'overflow-y', 'font-size', 'letter-spacing', 'word-spacing', 'line-height', 'text-indent']);
const BLOCKED_VALUE_SUBSTRINGS = ['url(', 'expression(', '@import', 'javascript:'];
const BLOCKED_PSEUDO_ELEMENTS = new Set(['::before', '::after', '::backdrop', '::marker',
// Single-colon legacy forms.
':before', ':after']);
const BLOCKED_AT_RULES = new Set(['import', 'font-face', 'keyframes', 'charset', 'namespace', 'supports', 'page', 'document', 'viewport']);
/**
 ...
 */
const validateSelector = rawSelector => {
  let dropReason = null;
...
const valueIsBlocked = rawValue => {
  const lowered = rawValue.toLowerCase();
  return BLOCKED_VALUE_SUBSTRINGS.some(needle => lowered.includes(needle));
};
const sanitizeDeclarations = (container, warnings) => {
  const toRemove = [];
  container.each(node => {
    if (node.type !== 'decl') {
      return;
...
const sanitizeRule = (rule, warnings) => {
...
const sanitizeAtRule = (atRule, warnings) => {
...
const sanitizeBrandingCss = input => {
  const warnings = [];
  if (input.trim() === '') {
    return {
      css: '',
...
export { ZSanitizeBrandingCssWarningSchema, sanitizeBrandingCss };
//# sourceMappingURL=sanitize-branding-css.js.map
