...
const validateCheckboxField = (values, fieldMeta, isSigningPage = false) => {
  const errors = [];
  const {
    readOnly,
    required,
    ...
    if (validation) {
      ...
      if (lengthCondition) {
        ...
        if (isSigningPage) {
          ...
          errorMessage = validation.value === '<=' ? `You need to select at least ${validationLength} options` : `You need to add at least ${validationLength} options`;
        }
...
const validateCheckboxLength = (numberOfSelectedOptions, validationRule, validationLength) => {
  return match(validationRule).with('>=', () => numberOfSelectedOptions >= validationLength).with('=', () => numberOfSelectedOptions === validationLength).with('<=', () => numberOfSelectedOptions <= validationLength).exhaustive();
};

export { validateCheckboxField, validateCheckboxLength };
//# sourceMappingURL=validate-checkbox.js.map
