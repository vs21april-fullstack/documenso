...
import { calculateMultiItemPosition, calculateFieldPosition } from './field-renderer.js';
...
const calculateRadioSize = fontSize => {
  return fontSize;
};
const renderRadioFieldElement = (field, options) => {
  const {
    pageWidth,
    pageHeight,
    pageLayer,
  ...
  const radioMeta = field.fieldMeta || null;
  ...
  radioValues.forEach(({
    ...
    const isRadioValueChecked = match(mode).with('edit', () => checked).with('sign', () => index.toString() === field.customText).with('export', () => {
...
export { renderRadioFieldElement };
//# sourceMappingURL=render-radio-field.js.map
