...
import { resolveFieldOverflowMode, FIELD_DEFAULT_GENERIC_VERTICAL_ALIGN, FIELD_DEFAULT_GENERIC_ALIGN, FIELD_DEFAULT_LETTER_SPACING, FIELD_DEFAULT_LINE_HEIGHT } from '../../types/field-meta.js';
import { calculateOverflowLayout } from './calculate-overflow-layout.js';
import { upsertFieldGroup, upsertFieldRect, createFieldHoverInteraction, konvaTextFill, konvaTextFontFamily } from './field-generic-items.js';
import { calculateFieldPosition } from './field-renderer.js';

const DEFAULT_TEXT_X_PADDING = 6;
const upsertFieldText = (field, options) => {
  const {
    pageWidth,
    pageHeight,
    mode = 'edit',
  ...
  const fieldText = pageLayer.findOne(`#${field.renderId}-text`) || new Konva.Text({
  ...
  let isLabel = true;
  let textToRender = fieldMeta?.label || fieldTypeName;
  let textAlign = 'center';
  let textVerticalAlign = FIELD_DEFAULT_GENERIC_VERTICAL_ALIGN;
...
const renderGenericTextFieldElement = (field, options) => {
  const {
    mode = 'edit',
    pageLayer,
    color
...
export { renderGenericTextFieldElement };
//# sourceMappingURL=render-generic-text-field.js.map
