...
import { getSignatureFontFamily, DEFAULT_SIGNATURE_TEXT_FONT_SIZE } from '../../constants/pdf.js';
import { AppError } from '../../errors/app-error.js';
import { resolveFieldOverflowMode } from '../../types/field-meta.js';
import { calculateOverflowLayout } from './calculate-overflow-layout.js';
import { upsertFieldGroup, upsertFieldRect, createFieldHoverInteraction } from './field-generic-items.js';
...
const getImageDimensions = (img, fieldWidth, fieldHeight) => {
 ...
 */
const createSignatureImage = (signatureImageAsBase64, fieldWidth, fieldHeight) => {
  if (typeof window !== 'undefined') {
    const img = new Image();
  ...
  const img = new SkiaImage(signatureImageAsBase64);
...
const createFieldSignature = (field, options) => {
  const {
    pageWidth,
    pageHeight,
    mode = 'edit',
  ...
  const textX = 0;
  const textY = 0;
  let textToRender = fieldTypeName;
...
const renderSignatureFieldElement = (field, options) => {
  const {
    mode = 'edit',
    pageLayer,
    pageWidth,
  ...
  const {
    ...
    isImageSignature,
    isLabel
  } = createFieldSignature(field, options);
...
export { renderSignatureFieldElement };
//# sourceMappingURL=render-signature-field.js.map
