 ...
 */
const calculateDropdownPosition = options => {
  const {
    fieldWidth,
    fieldHeight
  } = options;
...
const renderDropdownFieldElement = (field, options) => {
  const {
    pageWidth,
    pageHeight,
    pageLayer,
  ...
  const dropdownMeta = field.fieldMeta || null;
  ...
  if (dropdownMeta?.readOnly && dropdownMeta.defaultValue && dropdownMeta.values && dropdownMeta.values.some(value => value.value === dropdownMeta.defaultValue)) {
...
export { renderDropdownFieldElement };
//# sourceMappingURL=render-dropdown-field.js.map
