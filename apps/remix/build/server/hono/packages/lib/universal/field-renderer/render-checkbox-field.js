...
import { DEFAULT_STANDARD_FONT_SIZE } from '../../constants/pdf.js';
import { parseCheckboxCustomText } from '../../utils/fields.js';
import { upsertFieldGroup, upsertFieldRect, konvaTextFill, konvaTextFontFamily, createFieldHoverInteraction } from './field-generic-items.js';
import { calculateFieldPosition, calculateMultiItemPosition } from './field-renderer.js';
...
const calculateCheckboxSize = fontSize => {
  return fontSize;
};
const renderCheckboxFieldElement = (field, options) => {
  const {
    pageWidth,
    pageHeight,
    pageLayer,
  ...
  const checkboxMeta = field.fieldMeta || null;
  ...
  fieldGroup.on('transform', () => {
    ...
    const squares = fieldGroup.find('.checkbox-square').sort((a, b) => a.id().localeCompare(b.id(), undefined, {
      numeric: true
    }));
    const checkmarks = fieldGroup.find('.checkbox-checkmark').sort((a, b) => a.id().localeCompare(b.id(), undefined, {
      numeric: true
    }));
    const text = fieldGroup.find('.checkbox-text').sort((a, b) => a.id().localeCompare(b.id(), undefined, {
      numeric: true
    }));
  ...
  const checkedValues = field.customText ? parseCheckboxCustomText(field.customText) : [];
  checkboxValues.forEach(({
    value,
    checked
  }, index) => {
    const isCheckboxChecked = match(mode).with('edit', () => checked).with('sign', () => checkedValues.includes(index)).with('export', () => {
...
export { renderCheckboxFieldElement };
//# sourceMappingURL=render-checkbox-field.js.map
