...
const RECIPIENT_COLOR_STYLES = {
  readOnly: () => ({
    ...
    fieldItemInitials: '',
    comboBoxTrigger: 'ring-2 ring-recipient-green shadow-[0_0_0_5px_hsl(var(--recipient-green)/10%),0_0_0_2px_hsl(var(--recipient-green)/60%),0_0_0_0.5px_hsl(var(--recipient-green))]',
    comboBoxItem: ''
  }),
...
const generateStyles = recipientColor => {
  const {
    bg,
    border,
    ring,
...
const AVAILABLE_RECIPIENT_COLORS = ['green', 'blue', 'purple', 'orange', 'yellow', 'pink'];
({
  pattern: new RegExp(`(${Object.values(CSS_PROPERTY).join('|')})-recipient-(${AVAILABLE_RECIPIENT_COLORS.join('|')})(\\/(15|30))?$`)});
const getRecipientColor = index => {
  return AVAILABLE_RECIPIENT_COLORS[Math.max(index, 0) % AVAILABLE_RECIPIENT_COLORS.length];
};
const getRecipientColorStyles = colorOrIndex => {
  const color = typeof colorOrIndex === 'number' ? getRecipientColor(colorOrIndex) : colorOrIndex;
  return RECIPIENT_COLOR_STYLES[color]();
};

export { DEFAULT_RECT_BACKGROUND, getRecipientColor, getRecipientColorStyles };
//# sourceMappingURL=recipient-colors.js.map
