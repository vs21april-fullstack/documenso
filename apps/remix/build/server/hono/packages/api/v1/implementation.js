...
import { DATE_FORMATS, DEFAULT_DOCUMENT_DATE_FORMAT } from '../../lib/constants/date-formats.js';
import { EnvelopeType, SigningStatus, DocumentDataType, DocumentStatus } from '@prisma/client';
import { tsr } from '@ts-rest/serverless/fetch';
import { match } from 'ts-pattern';
import { TIME_ZONES, DEFAULT_DOCUMENT_TIME_ZONE } from '../../lib/constants/time-zones.js';
...
import { getEnvelopeById, getEnvelopeWhereInput } from '../../lib/server-only/envelope/get-envelope-by-id.js';
...
import { getRecipientsForDocument } from '../../lib/server-only/recipient/get-recipients-for-document.js';
...
import { extractDerivedDocumentEmailSettings } from '../../lib/types/document-email.js';
import { ZFieldMetaSchema, ZRadioFieldMeta, ZCheckboxFieldMeta, ZDropdownFieldMeta, ZNumberFieldMeta, ZTextFieldMeta } from '../../lib/types/field-meta.js';
...
import { getPresignPostUrl, getPresignGetUrl } from '../../lib/universal/upload/server-actions.js';
...
import { mapSecondaryIdToDocumentId, mapSecondaryIdToTemplateId } from '../../lib/utils/envelope.js';
import { prisma as prismaWithReplicas } from '../../prisma/index.js';
import { ApiContractV1 } from './contract.js';
import { authenticatedMiddleware } from './middleware/authenticated.js';

const ApiContractV1Implementation = tsr.router(ApiContractV1, {
  ...
  downloadSignedDocument: authenticatedMiddleware(async (args, user, team, {
    ...
    try {
      ...
      const {
        url
      } = await getPresignGetUrl(downloadOriginalDocument ? firstDocumentData.initialData : firstDocumentData.data);
      return {
        status: 200,
        body: {
          downloadUrl: url
  ...
  createDocument: authenticatedMiddleware(async (args, user, team, {
    ...
    try {
      ...
      const dateFormat = body.meta.dateFormat ? DATE_FORMATS.find(format => format.value === body.meta.dateFormat) : DATE_FORMATS.find(format => format.value === DEFAULT_DOCUMENT_DATE_FORMAT);
      ...
      const timezone = body.meta.timezone ? TIME_ZONES.find(tz => tz === body.meta.timezone) : DEFAULT_DOCUMENT_TIME_ZONE;
  ...
  createTemplate: authenticatedMiddleware(async (args, user, team, {
    ...
    try {
      ...
      const dateFormat = meta?.dateFormat ? DATE_FORMATS.find(format => format.value === meta?.dateFormat) : DATE_FORMATS.find(format => format.value === DEFAULT_DOCUMENT_DATE_FORMAT);
  ...
  createDocumentFromTemplate: authenticatedMiddleware(async (args, user, team, {
    ...
    const envelopeItems = await prismaWithReplicas.envelopeItem.findMany({
      where: {
  ...
  createField: authenticatedMiddleware(async (args, user, team, {
    ...
    try {
      const createdFields = await prismaWithReplicas.$transaction(async tx => {
        return Promise.all(fields.map(async fieldData => {
          const {
            recipientId,
            type,
            pageNumber,
          ...
          if (advancedField && !fieldMeta) {
            throw new Error('Field meta is required for this type of field. Please provide the appropriate field meta object.');
          }
          ...
          const result = match(type).with('RADIO', () => ZRadioFieldMeta.safeParse(fieldMeta)).with('CHECKBOX', () => ZCheckboxFieldMeta.safeParse(fieldMeta)).with('DROPDOWN', () => ZDropdownFieldMeta.safeParse(fieldMeta)).with('NUMBER', () => ZNumberFieldMeta.safeParse(fieldMeta)).with('TEXT', () => ZTextFieldMeta.safeParse(fieldMeta)).with('SIGNATURE', 'INITIALS', 'DATE', 'EMAIL', 'NAME', () => ({
            success: true,
            data: undefined
          })).with('FREE_SIGNATURE', () => ({
            success: false,
...
export { ApiContractV1Implementation };
