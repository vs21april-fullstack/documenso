import { symmetricEncrypt, symmetricDecrypt } from '../../../../lib/universal/crypto.js';
 ...
 * otherwise fail.
 */
const encryptCscToken = plaintext => {
  const key = requireEnv('NEXT_PRIVATE_ENCRYPTION_KEY');
  const hex = symmetricEncrypt({
    key,
    data: plaintext
  });
 ...
 */
const decryptCscToken = ciphertext => {
  const key = requireEnv('NEXT_PRIVATE_ENCRYPTION_KEY');
  try {
    const buf = symmetricDecrypt({
      key,
...
export { decryptCscToken, encryptCscToken };
