 ...
 */
const DEFAULT_RSA_DIGEST = 'SHA-256';
const HASH_OID_FOR_DIGEST = {
 ...
 */
const hashOidForDigest = digest => HASH_OID_FOR_DIGEST[digest];
const DIGEST_STRENGTH = {
  'SHA-256': 256,
  'SHA-384': 384,
  'SHA-512': 512
};
 ...
 */
const pickAlgorithmChoice = algoOids => {
  const candidates = [];
...
const resolveEcdsaCurve = curveOid => {
 ...
 */
const parseGeneralizedTime = value => {
 ...
 */
const policyToLibpdfSignerAlgo = policy => {
  ...
  const signatureAlgorithm = policy.signAlgoOid === '1.2.840.113549.1.1.10' ? 'RSA-PSS' : 'RSASSA-PKCS1-v1_5';
  return {
    keyType: 'RSA',
    signatureAlgorithm,
    digestAlgorithm: policy.digestAlgorithm,
...
export { hashOidForDigest, policyToLibpdfSignerAlgo, resolveCscAlgorithmPolicy };
//# sourceMappingURL=algorithm-resolver.js.map
