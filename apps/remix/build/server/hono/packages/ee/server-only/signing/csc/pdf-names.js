 ...
 * into the document.
 */
const hashToOpaqueSerial = label => bytesToHex(sha1(utf8ToBytes(label))).toUpperCase();
/** AcroForm signature-field name (TSP anchor) for a recipient + envelope item. */
const buildTspAnchorName = (recipientId, envelopeItemId) => hashToOpaqueSerial(`recipient:${recipientId}|item:${envelopeItemId}`);
/** `/Stamp` annotation name for a recipient + envelope item on a specific page. */
const buildTspStampName = (recipientId, envelopeItemId, pageNumber) => hashToOpaqueSerial(`recipient:${recipientId}|item:${envelopeItemId}|page:${pageNumber}`);

export { buildTspAnchorName, buildTspStampName };
//# sourceMappingURL=pdf-names.js.map
