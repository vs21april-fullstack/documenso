...
import { ZCscCredentialsListResponseSchema, ZCscCredentialsInfoResponseSchema } from './types.js';
 ...
 */
const cscCredentialsList = async opts => {
  const {
    baseUrl,
    accessToken,
    signal,
  ...
  if (response.credentialIDs.length === 0) {
    throw new AppError(AppErrorCode.CSC_CREDENTIAL_LIST_EMPTY, {
      message: 'CSC provider returned no credentials for the authenticated user. Recipient must enrol with the TSP before signing.'
    });
  }
  return response;
};
 ...
 */
const cscCredentialsInfo = async opts => {
  const {
    baseUrl,
    accessToken,
    signal,
...
export { cscCredentialsInfo, cscCredentialsList };
