...
import { buildTspAnchorName, buildTspStampName } from './pdf-names.js';
 ...
 */
const materializeTspAnchorsForEnvelope = async ({
  envelopeId
}) => {
  ...
  for (const envelopeItem of envelope.envelopeItems) {
    const expectedAnchorNames = envelope.recipients.map(recipient => buildTspAnchorName(recipient.id, envelopeItem.id));
    const expectedStampNames = [];
    ...
    await prismaWithReplicas.documentData.update({
      where: {
        id: envelopeItem.documentDataId
      },
      data: {
 ...
 */
const isAlreadyMaterialised = (pdfDoc, expectedAnchorNames, expectedStampNames) => {
  ...
  const presentStampNames = new Set();
...
export { materializeTspAnchorsForEnvelope };
//# sourceMappingURL=materialize-anchors.js.map
