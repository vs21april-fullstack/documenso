...
import { z } from 'zod';
import { CSC_OAUTH_FLOW_COOKIE_NAME, getCscCookieSecret, cscCookieBaseOptions } from './shared.js';
...
const ZCscOAuthFlowPayloadSchema = z.object({
...
const setCscOAuthFlowCookie = async options => {
  const {
    c,
    payload
  } = options;
 ...
 */
const getCscOAuthFlowCookie = async c => {
...
const clearCscOAuthFlowCookie = c => {
  deleteCookie(c, CSC_OAUTH_FLOW_COOKIE_NAME, cscCookieBaseOptions);
};

export { ZCscOAuthFlowPayloadSchema, clearCscOAuthFlowCookie, getCscOAuthFlowCookie, setCscOAuthFlowCookie };
//# sourceMappingURL=oauth-flow-cookie.js.map
