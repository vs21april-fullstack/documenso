...
import { ZCreateAttachmentRequestSchema, ZCreateAttachmentResponseSchema } from './create-attachment.types.js';

const createAttachmentRoute = authenticatedProcedure.meta({
  openapi: {
    method: 'POST',
...
}).input(ZCreateAttachmentRequestSchema).output(ZCreateAttachmentResponseSchema).mutation(async ({
  input,
  ctx
}) => {
  const {
