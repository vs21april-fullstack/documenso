...
import { ZUpdateOrganisationMemberRoleRequestSchema, ZUpdateOrganisationMemberRoleResponseSchema } from './update-organisation-member-role.types.js';
 ...
 */
const updateOrganisationMemberRoleRoute = adminProcedure.input(ZUpdateOrganisationMemberRoleRequestSchema).output(ZUpdateOrganisationMemberRoleResponseSchema).mutation(async ({
  input,
  ctx
}) => {
  const {
  ...
  const currentOrganisationRole = getHighestOrganisationRoleInGroup(member.organisationGroupMembers.flatMap(member => member.group));
  if (role === 'OWNER') {
...
export { updateOrganisationMemberRoleRoute };
//# sourceMappingURL=update-organisation-member-role.js.map
