...
import { OrganisationGroupType, TeamMemberRole } from '../../../prisma/generated/types.js';
import { match } from 'ts-pattern';
import { authenticatedProcedure } from '../trpc.js';
import { ZUpdateTeamMemberRequestSchema, ZUpdateTeamMemberResponseSchema } from './update-team-member.types.js';

const updateTeamMemberRoute = authenticatedProcedure
//   .meta(updateTeamMemberMeta)
.input(ZUpdateTeamMemberRequestSchema).output(ZUpdateTeamMemberResponseSchema).mutation(async ({
  ctx,
  input
}) => {
  const {
  ...
  const internalTeamGroupToRemoveMemberFrom = team.teamGroups.find(group => group.organisationGroup.type === OrganisationGroupType.INTERNAL_TEAM && group.teamId === teamId && group.organisationGroup.organisationGroupMembers.some(member => member.organisationMemberId === memberId));
  ...
  const {
    teamRole: currentMemberToUpdateTeamRole
  } = await getMemberRoles({
    teamId,
    reference: {
      type: 'Member',
  ...
  await prismaWithReplicas.$transaction(async tx => {
    ...
    await tx.organisationGroupMember.create({
      data: {
        ...
        organisationMemberId: memberId,
        groupId: match(data.role).with(TeamMemberRole.MEMBER, () => teamMemberGroup.organisationGroupId).with(TeamMemberRole.MANAGER, () => teamManagerGroup.organisationGroupId).with(TeamMemberRole.ADMIN, () => teamAdminGroup.organisationGroupId).exhaustive()
      }
    });
  });
});

export { updateTeamMemberRoute };
//# sourceMappingURL=update-team-member.js.map
