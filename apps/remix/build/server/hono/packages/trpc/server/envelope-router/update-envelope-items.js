...
import { updateEnvelopeItemsMeta, ZUpdateEnvelopeItemsRequestSchema, ZUpdateEnvelopeItemsResponseSchema } from './update-envelope-items.types.js';

const updateEnvelopeItemsRoute = authenticatedProcedure.meta(updateEnvelopeItemsMeta).input(ZUpdateEnvelopeItemsRequestSchema).output(ZUpdateEnvelopeItemsResponseSchema).mutation(async ({
  input,
  ctx
}) => {
  const {
  ...
  const itemsBelongToEnvelope = data.every(item => envelope.envelopeItems.some(({
    id
  }) => item.envelopeItemId === id));
...
export { updateEnvelopeItemsRoute };
