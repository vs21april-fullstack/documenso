...
import { ZSwapOrganisationSubscriptionRequestSchema, ZSwapOrganisationSubscriptionResponseSchema } from './swap-organisation-subscription.types.js';

const swapOrganisationSubscriptionRoute = adminProcedure.input(ZSwapOrganisationSubscriptionRequestSchema).output(ZSwapOrganisationSubscriptionResponseSchema).mutation(async ({
  input,
  ctx
}) => {
  const {
  ...
  const sourceOrg = await prismaWithReplicas.organisation.findUnique({
    where: {
  ...
  if (!sourceOrg.subscription || sourceOrg.subscription.status !== SubscriptionStatus.ACTIVE && sourceOrg.subscription.status !== SubscriptionStatus.PAST_DUE) {
    throw new AppError(AppErrorCode.INVALID_REQUEST, {
      message: 'Source organisation does not have an active subscription'
    });
  }
  const targetOrg = await prismaWithReplicas.organisation.findUnique({
    where: {
  ...
  if (targetOrg.subscription && (targetOrg.subscription.status === SubscriptionStatus.ACTIVE || targetOrg.subscription.status === SubscriptionStatus.PAST_DUE)) {
    throw new AppError(AppErrorCode.INVALID_REQUEST, {
  ...
  await prismaWithReplicas.$transaction(async tx => {
    ...
    if (sourceOrg.organisationClaim && targetOrg.organisationClaim) {
      await tx.organisationClaim.update({
        where: {
          id: targetOrg.organisationClaim.id
        },
        data: {
    ...
    if (sourceOrg.organisationClaim) {
      await tx.organisationClaim.update({
        where: {
          id: sourceOrg.organisationClaim.id
        },
        data: {
...
export { swapOrganisationSubscriptionRoute };
//# sourceMappingURL=swap-organisation-subscription.js.map
