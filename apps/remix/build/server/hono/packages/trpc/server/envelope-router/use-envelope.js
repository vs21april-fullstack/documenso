...
import { useEnvelopeMeta, ZUseEnvelopeRequestSchema, ZUseEnvelopeResponseSchema } from './use-envelope.types.js';

const useEnvelopeRoute = authenticatedProcedure.meta(useEnvelopeMeta).input(ZUseEnvelopeRequestSchema).output(ZUseEnvelopeResponseSchema).mutation(async ({
  input,
  ctx
}) => {
  const {
  ...
  const filesToUpload = files.filter((file, index) => payload.customDocumentData && payload.customDocumentData.some(mapping => mapping.identifier === file.name || mapping.identifier === index));
...
export { useEnvelopeRoute };
//# sourceMappingURL=use-envelope.js.map
