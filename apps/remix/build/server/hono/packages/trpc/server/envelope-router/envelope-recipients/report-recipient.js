...
import { reportSenderRateLimit } from '../../../../lib/server-only/rate-limit/rate-limits.js';
...
import { ZReportRecipientRequestSchema, ZReportRecipientResponseSchema } from './report-recipient.types.js';
 ...
 */
const reportRecipientRoute = procedure.input(ZReportRecipientRequestSchema).output(ZReportRecipientResponseSchema).mutation(async ({
  input,
  ctx
}) => {
  const {
  ...
  await prismaWithReplicas.organisationMonthlyStat.upsert({
    where: {
...
export { reportRecipientRoute };
//# sourceMappingURL=report-recipient.js.map
