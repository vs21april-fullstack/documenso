...
//   },
// };
const ZBulkMoveEnvelopesRequestSchema = z.object({
  envelopeIds: z.array(z.string()).min(1).max(100).describe('The IDs of the envelopes to move. The maximum number of envelopes you can move at once is 100.'),
  envelopeType: z.nativeEnum(EnvelopeType).describe('The type of the envelopes being moved.'),
  folderId: z.string().nullable().describe('The ID of the folder to move the envelopes to. If null envelopes will be moved to the root folder.')
});
const ZBulkMoveEnvelopesResponseSchema = z.object({
  movedCount: z.number().describe('The number of envelopes that were moved.')
});

export { ZBulkMoveEnvelopesRequestSchema, ZBulkMoveEnvelopesResponseSchema };
//# sourceMappingURL=bulk-move-envelopes.types.js.map
