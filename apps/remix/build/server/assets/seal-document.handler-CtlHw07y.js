...
import { GoogleKmsSigner, HttpTimestampAuthority, P12Signer, PDF, parsePem, rgb } from '@libpdf/core';
...
import {
  ...
  aJ as DEFAULT_HANDWRITING_FONT_SIZE,
  aK as DEFAULT_STANDARD_FONT_SIZE,
  m as DOCUMENT_AUDIT_LOG_TYPE,
  at as DOCUMENT_STATUS,
  aG as encryptSecondaryData,
  n as env,
  a2 as extractDocumentAuthMethods,
  aV as fieldsContainUnsignedRequiredField,
  au as formatDocumentAuditLogAction,
  aN as fromCheckboxValue,
  aC as getDocumentCertificateAuditLogs,
  h as getFileServerSide,
  aw as getOrganisationClaimByTeamId,
  aA as getSignatureFontFamily,
  a1 as getTeamSettings,
  ax as getTranslations,
  aY as isDocumentCompleted,
  aI as isSignatureFieldType,
  aW as isTspEnvelope,
  o as isValidLanguageCode,
  aL as MIN_HANDWRITING_FONT_SIZE,
  aM as MIN_STANDARD_FONT_SIZE,
  Q as mapDocumentIdToSecondaryId,
  ag as mapEnvelopeToWebhookDocumentPayload,
  aZ as NEXT_PRIVATE_DISABLE_PDF_SIGNING,
  as as NEXT_PRIVATE_INTERNAL_WEBAPP_URL,
  aq as NEXT_PRIVATE_SIGNING_TIMESTAMP_AUTHORITY,
  aE as NEXT_PRIVATE_USE_LEGACY_SIGNING_SUBFILTER,
  aX as NEXT_PRIVATE_USE_PLAYWRIGHT_PDF,
  aF as NEXT_PUBLIC_SIGNING_CONTACT_INFO,
  N as NEXT_PUBLIC_WEBAPP_URL,
  aD as PDF_SIZE_A4_72PPI,
  ay as parseDocumentAuditLogData,
  a8 as prefixedId,
  p as prismaWithReplicas,
  ar as putPdfFileServerSide,
  aB as RECIPIENT_ROLE_SIGNING_REASONS,
  R as RECIPIENT_ROLES_DESCRIPTION,
  aU as renderField,
  az as svgToPng,
  ae as triggerWebhook,
  aH as USE_INTERNAL_URL_BROWSERLESS,
  ai as ZCheckboxFieldMeta,
  aR as ZDateFieldMeta,
  aQ as ZEmailFieldMeta,
  aO as ZInitialsFieldMeta,
  aP as ZNameFieldMeta,
  aS as ZNumberFieldMeta,
  ah as ZRadioFieldMeta,
  av as ZSupportedLanguageCodeSchema,
  aT as ZTextFieldMeta,
  af as ZWebhookDocumentSchema,
...
import { groupBy, once, prop, sortBy } from 'remeda';
import { Canvas, DOMMatrix, FontLibrary, Image, Path2D } from 'skia-canvas';
...
const renderOverviewCard = (options) => {
  const { envelope, envelopeItems, envelopeOwner, recipients, width, i18n: i18n2 } = options;
  ...
  const recipientsLabel = renderOverviewCardLabels({
    ...
    text: recipients.map(
      (recipient) =>
        `[${i18n2._(RECIPIENT_ROLES_DESCRIPTION[recipient.role].roleName)}] ${recipient.name} (${recipient.email})`,
    ),
    width: columnWidth,
    groupX: columnWidth + columnSpacing,
  });
...
const renderRow$1 = (options) => {
  const { auditLog, width, i18n: i18n2 } = options;
...
const groupRowsIntoPages$1 = (options) => {
  const { auditLogs, maxHeight, contentWidth, i18n: i18n2, overviewCard } = options;
  const groupedRows = [[]];
...
const renderPages = (options) => {
  const { groupedRows, margin, pageTopMargin: pageTopMargin2, i18n: i18n2, overviewCard } = options;
...
const getAuditLogIndicatorColor = (type) =>
  match(type)
    .with(DOCUMENT_AUDIT_LOG_TYPE.DOCUMENT_RECIPIENT_COMPLETED, () => '#22c55e')
    .with(DOCUMENT_AUDIT_LOG_TYPE.DOCUMENT_RECIPIENT_REJECTED, () => '#ef4444')
    .with(DOCUMENT_AUDIT_LOG_TYPE.DOCUMENT_SENT, () => '#f97316')
    .with(
...
const renderRowHeader = (options) => {
  const { columnWidths, i18n: i18n2 } = options;
...
const renderColumnOne = (options) => {
  const { recipient, width, i18n: i18n2 } = options;
...
const renderColumnThree = (options) => {
  const { recipient, width, i18n: i18n2, envelopeOwner } = options;
  const column = new Konva.Group();
  const itemsToRender = [
    {
      label: i18n2._(
...
const renderRow = (options) => {
  const { recipient, columnWidths, i18n: i18n2, envelopeOwner } = options;
  const rowGroup = new Konva.Group();
...
const renderBranding = async ({ qrToken, i18n: i18n2 }) => {
  const branding = new Konva.Group();
...
const groupRowsIntoPages = (options) => {
  const { recipients, maxHeight, i18n: i18n2, columnWidths, envelopeOwner } = options;
...
const renderTables = (options) => {
  const { groupedRows, columnWidths, i18n: i18n2 } = options;
  const tables = [];
...
const generateCertificatePdf = async (options) => {
  ...
  const payload = {
    recipients: recipients.map((recipient) => {
      ...
      const emailSent = auditLogs['EMAIL_SENT'].find(
      ...
      const documentOpened = auditLogs['DOCUMENT_OPENED'].find(
        (log) => log.type === 'DOCUMENT_OPENED' && log.data.recipientId === recipientId,
      );
      const documentRecipientCompleted = auditLogs['DOCUMENT_RECIPIENT_COMPLETED'].find(
        (log) => log.type === 'DOCUMENT_RECIPIENT_COMPLETED' && log.data.recipientId === recipientId,
      );
      const documentRecipientRejected = auditLogs['DOCUMENT_RECIPIENT_REJECTED'].find(
...
const getAuditLogsPdf = async ({ documentId, language }) => {
...
const getCertificatePdf = async ({ documentId, language }) => {
  const { chromium } = await import('playwright');
  const encryptedId = encryptSecondaryData({
    data: documentId.toString(),
    expiresAt: DateTime.now()
...
const insertFieldInPDFV1 = async (pdf, field) => {
  ...
  if (isDebugMode) {
    ...
    if (pageRotationInDegrees !== 0) {
      const adjustedPosition = adjustPositionForRotation$1(
        pageWidth,
        pageHeight,
        debugX,
        debugY,
  ...
  await match(field)
    ...
    .otherwise((field2) => {
      ...
      const isMultiline =
        field2.type === FieldType.TEXT && (textWidth > fieldWidth || field2.customText.includes('\n'));
      const padding = 8;
...
const insertFieldInPDFV2 = async ({ pageWidth, pageHeight, fields }) => {
  ensureFontLibrary();
  let stage = new Konva.Stage({
    width: pageWidth,
    height: pageHeight,
...
const run = async ({ payload, io }) => {
  ...
  if (shouldSendCompletedEmail) {
    await io.runTask('send-document-completed-emails', async () => {
      const completedEmailHandler = await import('./send-document-completed-emails.handler-BomX36BQ.js');
...
const decorateAndSignPdf = async ({
  ...
  if (envelope.internalVersion === 1) {
    const legacy_pdfLibDoc = await PDFDocument.load(
      await pdfDoc.save({
        useXRefStream: true,
      }),
    );
