...
import { z } from 'zod';
import { g as getEmailContext } from './get-email-context-DGocVVjC.js';
import {
  g as getI18nInstance,
  r as renderEmailWithI18N,
  T as TemplateFooter,
} from './render-email-with-i18n-Cb4vxeu5.js';
import {
  w as AppError,
  x as AppErrorCode,
  g as assertOrganisationRatesAndLimits,
  Y as buildTeamWhereQuery,
  T as CSC_INSTANCE_SIGNATURE_LEVEL,
  l as createDocumentAuditLogData,
  aa as createDocumentAuthOptions,
  a4 as createRecipientAuthOptions,
  ab as DEFAULT_DOCUMENT_DATE_FORMAT,
  m as DOCUMENT_AUDIT_LOG_TYPE,
  a9 as extractDerivedDocumentMeta,
  a1 as extractDocumentAuthMethods,
  W as getEnvelopeWhereInput,
  h as getFileServerSide,
  X as getOrganisationTemplateWhereInput,
  a0 as getTeamSettings,
  aj as getTemplateById,
  Q as IS_INSTANCE_CSC_MODE,
  a5 as insertFormValuesInPdf,
  M as mapDocumentIdToSecondaryId,
  af as mapEnvelopeToWebhookDocumentPayload,
  $ as mapSecondaryIdToTemplateId,
  N as NEXT_PUBLIC_WEBAPP_URL,
  a7 as prefixedId,
  p as prismaWithReplicas,
  a6 as putNormalizedPdfFileServerSide,
  V as SignatureLevel,
  ak as sendDocument,
  ad as triggerWebhook,
  ah as ZCheckboxFieldMeta,
  a2 as ZDefaultRecipientsSchema,
  ai as ZDropdownFieldMeta,
  ac as ZFieldMetaSchema,
  ag as ZRadioFieldMeta,
  a3 as ZRecipientAuthOptionsSchema,
  a8 as ZSignatureLevelSchema,
...
import 'stripe';
import 'jose';
const BulkSendCompleteEmail = ({ userName, templateName, totalProcessed, successCount, failedCount, errors }) => {
  const { _ } = useLingui();
  const previewText =
    /*i18n*/
    {
                        ...
                        children: [
                          ...
                          /* @__PURE__ */ jsx('ul', {
                            ...
                            children: errors.map((error, index) =>
                              /* @__PURE__ */ jsx(
                                'li',
                                { className: 'mt-1 text-destructive text-sm', children: error },
                                index,
                              ),
                            ),
                          }),
...
const resolveSignatureLevel = ({ requested, strict = false } = {}) => {
...
const createDocumentFromTemplate = async ({
  ...
  const { envelope, createdEnvelope } = await prismaWithReplicas.$transaction(async (tx) => {
    ...
    if (prefillFields?.length) {
      const invalidFieldIds = prefillFields
        ...
        .filter((id2) => !allTemplateFieldIds.includes(id2));
