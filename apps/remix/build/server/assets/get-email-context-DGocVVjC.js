...
import { z } from 'zod';
import {
  M as MailChannelsTransport,
  m as mailer,
  b as resolveEmailBrandingColors,
} from './render-email-with-i18n-Cb4vxeu5.js';
import {
  w as AppError,
  x as AppErrorCode,
  s as DOCUMENSO_ENCRYPTION_SECONDARY_KEY,
  D as DOCUMENSO_INTERNAL_EMAIL,
  y as extractDerivedTeamSettings,
  I as IS_BILLING_ENABLED,
  v as logger,
  N as NEXT_PUBLIC_WEBAPP_URL,
  p as prismaWithReplicas,
  t as symmetricDecrypt,
  Z as ZCssVarsSchema,
...
const ZEmailTransportConfigSchema = z.discriminatedUnion('type', [
...
const getEmailContext = async (options) => {
  ...
  const senderEmailId = match(meta?.emailId)
    .with(P.string, (emailId) => emailId)
    .with(void 0, () => emailContext.settings.emailId)
    .with(null, () => null)
    .exhaustive();
