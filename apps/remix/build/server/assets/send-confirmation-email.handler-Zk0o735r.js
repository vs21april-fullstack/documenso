...
import {
  ...
  U as USER_SIGNUP_VERIFICATION_TOKEN_IDENTIFIER,
...
const TemplateConfirmationEmail = ({ confirmationLink, assetBaseUrl }) => {
...
const ConfirmEmailTemplate = ({ confirmationLink, assetBaseUrl = 'http://localhost:3002' }) => {
  const { _ } = useLingui();
  const previewText =
    /*i18n*/
    {
...
const getMostRecentEmailVerificationToken = async ({ userId }) => {
...
const sendConfirmationToken = async ({ email, force = false }) => {
