...
import { p as prismaWithReplicas, e as unsafeBuildEnvelopeIdQuery, h as getFileServerSide, E as EMAIL_ASSET_BASE_URL, j as PUBLISHED_APP_URL, k as formatDocumentsPath, f as extractDerivedDocumentEmailSettings, l as createDocumentAuditLogData, m as DOCUMENT_AUDIT_LOG_TYPE, i as isRecipientEmailValidForSending, g as assertOrganisationRatesAndLimits } from "./server-build-BldIODWZ.js";
...
const run = async ({
  ...
  await Promise.all(recipientsToNotify.map(async (recipient) => {
    ...
    const reportUrl = recipient.role === RecipientRole.CC ? `${PUBLISHED_APP_URL()}report/${recipient.token}` : void 0;
    const template = createElement(DocumentCompletedEmailTemplate, {
      documentName: envelope.title,
      assetBaseUrl,
