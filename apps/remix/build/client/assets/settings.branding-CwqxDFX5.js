import { A as f, a as h, b as x } from './alert-CE06zyBg.js';
import { I as u } from './app-uAhqo_pP.js';
import { B as Q } from './branding-preferences-form-BGGlS8ox.js';
import { B as W } from './button-Dm_JGgap.js';
import { L as D, w as E, r as R } from './chunk-KS7C4IRE-FYOnnPbz.js';
import { t as l } from './index-BbY_VZqi.js';
import { i as M } from './index-CBoJQWs5.js';
import { T as e, u as O } from './index-CkOHfBoV.js';
import { j as t } from './jsx-runtime-DrYFQjIW.js';
import { L as J } from './loader-aSxoB_gm.js';
import { u as F } from './organisation-Ylr4XBmr.js';
import { c as _ } from './organisations-CAYnRnac.js';
import { S as U } from './settings-header-C7I7HmzF.js';
import { a as G } from './team-BvmzkpNi.js';
import { u as P } from './use-toast-pMYLxjk1.js';
import './index-BiLbLflF.js';
import './types-GU3YNY2F.js';
import './time-zones-DGI2eNDr.js';
import './envelope-reminder-YclQYJ2m.js';
import './organisations-bLnymRV_.js';
import './index-browser-CnbpKWEs.js';
import './document-email-DcBZA0tR.js';
import './data-transformer-DaTnp7WB.js';
import './index-DJ-EwNXm.js';
import './useQuery-Cl3ReeOA.js';
import './utils-C68LRSOY.js';
import './url-CP0Hgou8.js';
import './css-vars-MUIo1hcP.js';
import './accordion-DtuzNAcG.js';
import './index-DzY7AtIB.js';
import './index-DAyHhKCk.js';
import './index-J1tJADds.js';
import './index-DXkD4Uno.js';
import './index-ZximTWpF.js';
import './index-C829O7aq.js';
import './index-UGPUXo4T.js';
import './index-_mbtlgCZ.js';
import './chevron-down-mVGROlJc.js';
import './createLucideIcon-UOjYlZt5.js';
import './color-picker-DFvMRl3R.js';
import './popover-CGHMP7Nz.js';
import './Combination-DaG98sNE.js';
import './index-DoLGTOH8.js';
import './index-KpgwOPga.js';
import './form-BX2-nwLu.js';
import './index-BlIM-koY.js';
import './proxy-dEM8-k5c.js';
import './input-DdDbBJYb.js';
import './select-Dvz92dRn.js';
import './animate-generic-fade-in-out-CGjjMFem.js';
import './index-MjUwLcg-.js';
import './index-CZmIGLNu.js';
import './check--7Y29bNj.js';
import './textarea-D4wUhXuy.js';
import './zod-C00iebzH.js';
import './nonce-Dwj7D0fA.js';
import './form-sticky-save-bar-thblyFdT.js';
import './triangle-alert-Y8Kk1Rgi.js';
const kt = E(() => {
  const o = G(),
    n = F(),
    { _: i } = O(),
    { toast: m } = P(),
    [c, j] = R.useState([]),
    { data: g, isLoading: b, refetch: L } = l.team.get.useQuery({ teamReference: o.id }),
    { mutateAsync: v } = l.team.settings.update.useMutation(),
    { mutateAsync: C } = l.team.settings.updateBrandingLogo.useMutation(),
    N = n.organisationClaim.flags.allowCustomBranding || !u(),
    y = n.organisationClaim.flags.embedSigningWhiteLabel === !0 || !u(),
    B = async (r) => {
      try {
        const {
          brandingEnabled: s,
          brandingLogo: a,
          brandingUrl: w,
          brandingCompanyDetails: S,
          brandingColors: A,
          brandingCss: T,
        } = r;
        if (a instanceof File || a === null) {
          const p = new FormData();
          p.append('payload', JSON.stringify({ teamId: o.id })),
            a instanceof File && p.append('brandingLogo', a),
            await C(p);
        }
        const I = await v({
          teamId: o.id,
          data: {
            brandingEnabled: s,
            brandingUrl: w || null,
            brandingCompanyDetails: S || null,
            brandingColors: A,
            brandingCss: T,
          },
        });
        await L();
        const d = I?.cssWarnings ?? [];
        j(d),
          d.length > 0
            ? m({
                title: i({ id: 'yfhRAv' }),
                description: M._({ id: 'o0HRfL', values: { 0: d.length } }),
                duration: 8e3,
              })
            : m({ title: i({ id: '4t+Ruo' }), description: i({ id: 'd/M34U' }) });
      } catch (s) {
        throw (m({ title: i({ id: 'nwtY4N' }), description: i({ id: 'sXQQyg' }), variant: 'destructive' }), s);
      }
    };
  return b || !g
    ? t.jsx('div', {
        className: 'flex items-center justify-center rounded-lg py-32',
        children: t.jsx(J, { className: 'h-6 w-6 animate-spin text-muted-foreground' }),
      })
    : t.jsxs('div', {
        className: 'max-w-2xl',
        children: [
          t.jsx(U, { title: i({ id: 'ev3nK5' }), subtitle: i({ id: 'WZTU18' }) }),
          N
            ? t.jsxs('section', {
                children: [
                  t.jsx(Q, {
                    canInherit: !0,
                    hasAdvancedBranding: y,
                    context: 'Team',
                    settings: g.teamSettings,
                    onFormSubmit: B,
                  }),
                  c.length > 0 &&
                    t.jsxs(f, {
                      variant: 'warning',
                      className: 'mt-6',
                      children: [
                        t.jsx(x, { children: t.jsx(e, { id: 'RFDjVV' }) }),
                        t.jsx(h, {
                          children: t.jsx('ul', {
                            className: 'list-disc pl-5',
                            children: c.map((r, s) =>
                              t.jsxs(
                                'li',
                                {
                                  children: [
                                    r.detail,
                                    r.line !== void 0 &&
                                      t.jsxs('span', {
                                        className: 'text-muted-foreground',
                                        children: [' ', t.jsx(e, { id: 'PYQZ/s', values: { 0: r.line } })],
                                      }),
                                  ],
                                },
                                s,
                              ),
                            ),
                          }),
                        }),
                      ],
                    }),
                ],
              })
            : t.jsxs(f, {
                className: 'mt-8 flex flex-col justify-between p-6 sm:flex-row sm:items-center',
                variant: 'neutral',
                children: [
                  t.jsxs('div', {
                    className: 'mb-4 sm:mb-0',
                    children: [
                      t.jsx(x, { children: t.jsx(e, { id: 'ev3nK5' }) }),
                      t.jsx(h, { className: 'mr-2', children: t.jsx(e, { id: 'psC2J0' }) }),
                    ],
                  }),
                  _('MANAGE_BILLING', n.currentOrganisationRole) &&
                    t.jsx(W, {
                      asChild: !0,
                      variant: 'outline',
                      children: t.jsx(D, { to: `/o/${n.url}/settings/billing`, children: t.jsx(e, { id: 'J4ERpS' }) }),
                    }),
                ],
              }),
        ],
      });
});
export { kt as default };
