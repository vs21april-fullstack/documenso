import { b as F, w as j, r as m, u as V } from './chunk-KS7C4IRE-FYOnnPbz.js';
import { C as L, a as M, b as W } from './configure-fields-view-DYHrNCoc.js';
import { S as N } from './document-flow-root-C_DLP3ot.js';
import { Z as k } from './embed-authoring-base-schema-CJ09OeOv.js';
import { t as A } from './index-BbY_VZqi.js';
import { u as $ } from './index-CkOHfBoV.js';
import { j as a } from './jsx-runtime-DrYFQjIW.js';
import { p as B } from './put-file-CoKS3pQz.js';
...
const tr = j(() => {
  const { _: i } = $(),
    ...
    d = F(),
    [u] = V(),
    f = u.get('token') ?? void 0,
    [t, g] = m.useState(null),
    [h, x] = m.useState(null),
    [w, I] = m.useState(null),
    [S, b] = m.useState(null),
    [v, n] = m.useState(1),
    { mutateAsync: E } = A.embeddingPresign.createEmbeddingTemplate.useMutation(),
    y = (e) => {
      g(e), n(2);
    },
    C = (e) => {
    ...
    P = async (e) => {
      try {
        if (!t || !t.documentData) {
          s({ variant: 'destructive', title: i('Error'), description: i('Please configure the template first') });
          return;
        }
        const r = e.fields,
          D = await B(
            {
              arrayBuffer: async () => Promise.resolve(t.documentData.data.buffer),
              name: t.documentData.name,
              type: t.documentData.type,
            },
            { presignToken: f },
          ),
          c = { ...t.meta, externalId: S || t.meta.externalId },
          l = await E({
            title: t.title,
            documentDataId: D.id,
            meta: c,
            recipients: t.signers.map((p) => ({
              name: p.name,
              email: p.email,
              role: p.role,
              fields: r
                .filter((o) => o.signerEmail === p.email)
                .map((o) => ({ ...o, pageX: o.pageX, pageY: o.pageY, width: o.pageWidth, height: o.pageHeight })),
            })),
          });
        s({ title: i('Success'), description: i('Template created successfully') }),
          window.parent !== window &&
            window.parent.postMessage(
              { type: 'template-created', templateId: l.templateId, externalId: c.externalId },
              '*',
            );
        const T = window.location.hash.slice(1);
        await d(`/embed/v1/authoring/completed/create?templateId=${l.templateId}&externalId=${c.externalId}#${T}`);
      } catch (r) {
        console.error('Error creating template:', r),
          s({ variant: 'destructive', title: i('Error'), description: i('Failed to create template') });
      }
    };
  return (
    m.useLayoutEffect(() => {
      try {
        const e = window.location.hash.slice(1),
          r = k.safeParse(JSON.parse(decodeURIComponent(atob(e))));
        if (!r.success) {
          return;
        }
        I(r.data.features), r.data.externalId && b(r.data.externalId);
      } catch (e) {
        console.error('Error parsing embedding params:', e);
      }
    }, []),
    a.jsx('div', {
      ...
      children: a.jsx(L, {
        ...
        children: a.jsxs(N, {
          ...
          setCurrentStep: n,
          children: [
            a.jsx(M, { type: 'template', defaultValues: t ?? void 0, onSubmit: y }),
            a.jsx(W, { configData: t, defaultValues: h ?? void 0, onBack: C, onSubmit: P }),
          ],
        }),
      }),
    })
