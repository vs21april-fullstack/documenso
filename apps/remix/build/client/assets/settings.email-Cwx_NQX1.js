...
import { E as S } from './email-preferences-form-DSBYf-yD.js';
import { t as p } from './index-BbY_VZqi.js';
import { u as x } from './index-CkOHfBoV.js';
import { j as i } from './jsx-runtime-DrYFQjIW.js';
import { a as j } from './meta-1txnm5en.js';
import { S as T } from './settings-header-C7I7HmzF.js';
import { S as l } from './spinner-CEVyI0yx.js';
...
const Et = f(() => {
  const { _: t } = x(),
    { toast: o } = g(),
    m = h(),
    { data: e, isLoading: s } = p.team.get.useQuery({ teamReference: m.url }),
    { mutateAsync: a } = p.team.settings.update.useMutation(),
    n = async (u) => {
      try {
        const { emailId: r, emailReplyTo: c, emailDocumentSettings: d } = u;
        await a({ teamId: m.id, data: { emailId: r, emailReplyTo: c, emailDocumentSettings: d } }),
          o({ title: t({ id: 'gpKzYy' }), description: t({ id: 'Y9wW3z' }) });
      } catch (r) {
        throw (o({ title: t({ id: 'ywDBfs' }), description: t({ id: 'U5E5kb' }), variant: 'destructive' }), r);
      }
    };
  return s || !e
    ? i.jsx(l, {})
    : i.jsxs('div', {
        className: 'max-w-2xl',
        children: [
          i.jsx(T, { title: t({ id: '0fzruQ' }), subtitle: t({ id: 'nBgiaO' }) }),
          i.jsx('section', { children: i.jsx(S, { canInherit: !0, settings: e.teamSettings, onFormSubmit: n }) }),
        ],
      });
});
export { Et as default, yt as meta };
