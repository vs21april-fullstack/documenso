...
Omni Sign supports any identity provider that implements the OpenID Connect (OIDC) protocol. Common providers include:
...
- **Redirect URI**: The value from the Omni Sign SSO settings page
...
Back in Omni Sign, enter the following:
...
Omni Sign verifies their identity with your provider.
...
- Cannot set a Omni Sign password (they must use SSO)
...
<Callout type="info">
  SSO-only enforcement is not currently available. Users who have an existing Omni Sign account with a password can still sign in with their original method in addition to SSO.
</Callout>
...
<Accordions type="multiple">
  ...
  - Copy the redirect URI exactly from Omni Sign's SSO settings
  ...
  <Accordion title="User already exists">
    If a user already has a Omni Sign account with the same email, their existing account will be linked to your team and they can sign in through your SSO portal going forward.
  </Accordion>
</Accordions>
