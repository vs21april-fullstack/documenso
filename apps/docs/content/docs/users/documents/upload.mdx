...
description: Upload documents to Omni Sign to prepare them for signing. Covers supported formats, file size limits, and upload methods.
---
...
<Callout type="warn">
  Omni Sign does not support password-protected or encrypted PDF files. Remove encryption before
  uploading.
</Callout>

## Supported Formats

Omni Sign accepts two file formats:
...
If you do upload a `.docx` file, Omni Sign converts it to PDF before adding it to the envelope. The original `.docx` is discarded. Only the converted PDF is stored, signed, and downloaded.
...
- **Form controls are flattened.** Word content controls (drop-downs, date pickers, checkboxes) become static text or graphics. Use Omni Sign fields for anything that needs to be filled in.
...
1. **Processing**: Omni Sign validates the PDF and extracts page information
...
<Accordion title="Only PDF and DOCX files are allowed">
  Omni Sign accepts PDF and DOCX. For other formats (`.doc`, `.odt`, `.rtf`, etc.), export to PDF
