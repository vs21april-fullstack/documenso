...
## How Omni Sign Supports Compliance

Omni Sign provides features that support compliance with e-signature laws across jurisdictions:
...
Omni Sign maintains an audit trail for each document, recording:
...
## What Omni Sign Does NOT Provide

Omni Sign supports compliance with Simple Electronic Signature (SES) requirements. The following are not currently provided:
