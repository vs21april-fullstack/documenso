---
title: Kubernetes
description: Deploy Omni Sign on Kubernetes for production environments requiring high availability, auto-scaling, and enterprise-grade infrastructure.
---
...
A typical Omni Sign Kubernetes deployment consists of:
...
Create a dedicated namespace for Omni Sign:
...
Create the Omni Sign Deployment:
...
Update your Omni Sign secret to use the in-cluster database:
...
Omni Sign stores documents in the database by default. For high-volume deployments, configure S3-compatible storage.
...
Scale Omni Sign based on CPU and memory utilization:
