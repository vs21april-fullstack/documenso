---
title: Docker
description: Run Omni Sign as a standalone Docker container with an external database. Ideal for container platforms like AWS ECS, Google Cloud Run, or Azure Container Instances.
---
...
The Omni Sign image is available on both DockerHub and GitHub Container Registry:
...
These variables must be set for Omni Sign to start:
...
| `NEXT_PUBLIC_WEBAPP_URL`                | Public URL where Omni Sign is accessible (e.g., `https://sign.example.com`) | -                             |
...
Omni Sign requires two database connection strings:
...
docker run -d \
  ...
  -e NEXT_PRIVATE_SMTP_FROM_NAME="Omni Sign" \
...
Omni Sign provides health check endpoints for monitoring:
...
To update Omni Sign to a new version:
...
The Omni Sign container is stateless. All persistent data is stored in the PostgreSQL database.
