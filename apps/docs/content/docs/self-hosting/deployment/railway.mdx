---
title: Railway
description: Deploy Omni Sign on Railway with automatic builds from Git, managed PostgreSQL, and built-in networking. Use the one-click template or manual setup.
---
...
The fastest way to deploy Omni Sign on Railway is using the official template:
...
Click the URL in the Railway dashboard to access your Omni Sign instance.
...
If you prefer more control over the setup, deploy Omni Sign manually:
...
- Fork or select the [Omni Sign repository](https://github.com/documenso/documenso)
...
Click on the Omni Sign service and go to the "Variables" tab. Add the following variables:
...
NEXT_PRIVATE_SMTP_FROM_NAME=Omni Sign
...
Omni Sign runs database migrations automatically on startup.
...
- Click on your Omni Sign service
...
Railway provides several options for scaling your Omni Sign deployment:
...
<Steps>
<Step>
### Open your Omni Sign service

In the Railway dashboard, select the Omni Sign service.
...
In the Railway dashboard, select your Omni Sign service and open "Settings".
...
<Callout type="warn">
  Omni Sign stores sessions in the database, so horizontal scaling works without additional
...
Railway uses usage-based pricing. Estimated monthly costs for Omni Sign:
...
Omni Sign requires a signing certificate to sign documents. On Railway, you can:

### Option 1: Use Generated Certificate (Default)

Omni Sign generates a self-signed certificate automatically if none is provided. This works for testing but shows generic certificate information in signed documents.
...
In your Omni Sign service variables, set:
