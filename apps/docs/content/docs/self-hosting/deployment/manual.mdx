...
description: Deploy Omni Sign on a Linux server without Docker using Node.js and systemd.
---
...
<Callout type="info">
  If you use a reverse proxy in front of Omni Sign, set `NEXT_PUBLIC_WEBAPP_URL` to the public URL
...
Create a service file to run Omni Sign as a background service on Linux:

```ini
# /etc/systemd/system/documenso.service

[Unit]
Description=Omni Sign
...
A minimal Nginx configuration for proxying to Omni Sign:
