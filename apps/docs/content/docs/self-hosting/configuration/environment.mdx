...
description: Complete reference for all environment variables used to configure a self-hosted Omni Sign instance.
---

## Required Variables

These variables must be set for Omni Sign to function:
...
| `NEXT_PUBLIC_WEBAPP_URL`                | Public URL of your Omni Sign instance (e.g., `https://sign.example.com`)                   |
...
Omni Sign requires PostgreSQL 14 or higher.
...
Before delivering a webhook, Omni Sign checks whether the target resolves to a
...
Omni Sign supports multiple email transports for sending notifications.
...
Omni Sign can store documents in the database or S3-compatible storage.
...
Omni Sign requires a certificate to digitally sign documents.

### Transport Selection

| Variable                         | Description                                       | Default |
| -------------------------------- | ------------------------------------------------- | ------- |
| `NEXT_PRIVATE_DISABLE_PDF_SIGNING` | Set to `true` to complete documents without a cryptographic PDF signature. Recipient fields and completion certificates are still rendered. | `false` |
...
These flags are opt-in: when none are set, signin behaviour is unchanged from a stock Omni Sign instance.
...
Omni Sign can use Google Vertex AI for recipient and field detection.
...
Omni Sign can accept `.docx` uploads by sending them to a [Gotenberg](https://gotenberg.dev) service that converts them to PDF. When `NEXT_PRIVATE_DOCUMENT_CONVERSION_URL` is unset, DOCX uploads are rejected and only PDFs are accepted.
...
Omni Sign supports multiple background job providers for processing emails, documents, webhooks, and scheduled tasks.
