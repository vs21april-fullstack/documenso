...
description: Enable DOCX uploads on a self-hosted Omni Sign instance by running a Gotenberg sidecar that converts Word documents to PDF.
---
...
Omni Sign can accept `.docx` uploads in addition to PDFs. When a user uploads a Word document, the Omni Sign server sends it to a [Gotenberg](https://gotenberg.dev) service which uses LibreOffice to convert it to PDF. The converted PDF is what gets stored, signed, and downloaded. The original DOCX is discarded.
...
| Output format           | PDF                                                                  |
| Network requirement     | Omni Sign must reach the Gotenberg HTTP API                          |
...
- Network reachability from the Omni Sign container to the Gotenberg HTTP API.
- A version of Omni Sign that includes the document conversion feature.
...
For better fidelity, especially for non-Latin scripts, build a derived image that adds Microsoft Core Fonts and additional language fonts. The Omni Sign repository ships a reference Dockerfile at [`docker/development/Dockerfile.gotenberg`](https://github.com/documenso/documenso/blob/main/docker/development/Dockerfile.gotenberg) that you can use as a starting point:
...
The Gotenberg service should run **alongside** your Omni Sign container, not exposed to the public internet. The conversion service has no built-in authorisation beyond HTTP Basic auth, so it should sit on a private network or behind your existing reverse proxy.

<Tabs items={['Docker Compose', 'Kubernetes', 'External Instance']}>
<Tab value="Docker Compose">

Add a `gotenberg` service to the `compose.yml` you use for Omni Sign:
...
Do **not** publish Gotenberg's port (`3000`) to the host. Omni Sign reaches it over the internal Docker network using the service name (`http://gotenberg:3000`).
...
Then reference the in-cluster URL from Omni Sign's environment:
...
Omni Sign does not have to colocate with Gotenberg. You can point it at any reachable Gotenberg deployment: a managed instance, a shared internal service, or a Gotenberg-compatible API.
...
- Be reachable from the Omni Sign container with low enough latency that the 30 second per-request timeout is comfortable.
...
| `--api-timeout=500s`              | Server-side request ceiling. Omni Sign aborts at 30 s by default, so this is a safety net for very large documents.                                                                                                                                                  |
...
| `--pdfengines-disable-routes`     | Disables the PDF engines routes Omni Sign does not use. Shrinks the attack surface.                                                                                                                                                                                  |
| `--webhook-disable`               | Disables webhook callbacks. Omni Sign uses synchronous requests only.                                                                                                                                                                                                |

## Configure Omni Sign

Set the following environment variables on the Omni Sign container and restart it.
...
<Steps>
<Step>
### Restart the Omni Sign container
...
From a shell inside the Omni Sign container or another container on the same network:
...
In the Omni Sign web UI, open **Documents** and try uploading a small `.docx` file. The upload dropzone should accept it, and after a few seconds the editor should open with the converted PDF.
...
- **Disable unused routes.** `--pdfengines-disable-routes` and `--webhook-disable` reduce attack surface. Omni Sign only uses the LibreOffice convert route.
...
- **Rotate credentials.** Rotating the basic auth secret is a config change in both Gotenberg and Omni Sign, followed by a restart of each.
...
<Accordions type="multiple">
  <Accordion title="DOCX uploads are rejected with 'Only PDF and DOCX files are allowed'">
    The Omni Sign server does not see `NEXT_PRIVATE_DOCUMENT_CONVERSION_URL`. Check the value is set
  ...
  <Accordion title="Uploads fail with 'Document conversion service is currently unavailable'">
    Omni Sign could not reach Gotenberg. Verify:

    - The URL in `NEXT_PRIVATE_DOCUMENT_CONVERSION_URL` is resolvable from the Omni Sign container
  ...
  <Accordion title="Form controls in the DOCX appear blank or missing">
    Omni Sign disables Gotenberg's `exportFormFields` flag during conversion. Word content controls
    (`<w:sdt>`) become static graphics in the output PDF, which prevents Omni Sign's later
    flattening step from making them invisible. This is intentional. Use Omni Sign fields
