...
Omni Sign supports four email transport methods:
...
SMTP is the most common email transport. Omni Sign supports two SMTP authentication methods.
...
Generate an API key in the Resend dashboard for Omni Sign to use.
...
Omni Sign uses React Email templates for all outgoing emails. The templates are located in `packages/email/templates/` and include:
...
- Custom logo replaces the Omni Sign logo
...
Configure Omni Sign to use the test server:
...
NEXT_PRIVATE_SMTP_FROM_NAME="Omni Sign Test"
```
...
For MailChannels, configure DKIM directly in Omni Sign using the environment variables described above.
