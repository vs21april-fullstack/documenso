...
description: Configure PostgreSQL connection strings, pooling, SSL, migrations, and performance tuning for your Omni Sign deployment.
---
...
Omni Sign requires **PostgreSQL 14 or later**. No other databases are supported.
...
Omni Sign uses two database connection variables:
...
Omni Sign automatically detects common database environment variable formats used by hosting providers:
...
If your hosting provider sets these variables, Omni Sign will use them automatically.
...
Omni Sign uses Prisma, which maintains its own connection pool. Configure pool size with connection string parameters:
...
Database migrations update your schema when upgrading Omni Sign.

### Automatic Migrations

When running Omni Sign via Docker, migrations run automatically on container startup. No manual intervention is required.
...
Key parameters for Omni Sign workloads:
...
Omni Sign includes necessary indexes by default. Additional indexes may help for:
