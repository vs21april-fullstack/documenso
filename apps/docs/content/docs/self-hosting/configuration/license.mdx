...
- A running self-hosted Omni Sign instance that you're able to restart
...
If you're running Omni Sign directly (not in a container), add the variable to your `.env` file:
...
On startup, Omni Sign validates the key against the Omni Sign license server and caches the result locally for future startups, so a brief license-server outage won't lock you out.
