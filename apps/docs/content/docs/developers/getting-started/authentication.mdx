...
- A Omni Sign account (cloud or self-hosted)
- A Omni Sign account on any plan (Free, Individual, Team, or Enterprise)
...
- Log in to your Omni Sign account
...
Omni Sign provides official SDKs that handle authentication for you:

```typescript
import { Omni Sign } from '@documenso/sdk-typescript';

const client = new Omni Sign({
