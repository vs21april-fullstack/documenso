...
Verifying webhook requests ensures that incoming payloads originate from Omni Sign and have not been tampered with. Without verification, attackers could forge requests to your endpoint and trigger unintended actions.

## How Omni Sign Signs Webhooks

When you configure a webhook with a secret, Omni Sign includes that secret in every webhook request via the `X-Documenso-Secret` header. Your server should compare this header value against your stored secret to authenticate the request.
...
| --------------- | --------------------------------------- | ------------------------------------------------------------------ |
| Secret mismatch | Webhook secret changed or misconfigured | Verify the secret in your environment matches the one in Omni Sign |
| Empty header    | Webhook created without a secret        | Add a secret to the webhook configuration in Omni Sign             |
...
3. Update the webhook configuration in Omni Sign
