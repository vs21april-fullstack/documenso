...
Webhooks are HTTP callbacks triggered by specific events in Omni Sign. When an event occurs (such as a document being signed), Omni Sign sends an HTTP POST request to your configured URL with details about the event.
...
Before configuring a webhook in Omni Sign, you need an endpoint that can receive HTTP POST requests. Here's a minimal example:
...
<Callout type="warn">
  Always respond with a `200 OK` status within 30 seconds. Omni Sign will retry failed deliveries.
</Callout>

## Configuring Webhooks in Omni Sign via the Dashboard
...
Omni Sign provides a built-in testing feature to verify your webhook endpoint works correctly.
...
<Step>
Omni Sign will attempt to deliver the same payload again
</Step>
</Steps>

## Retry Policy

When a webhook delivery fails (non-2xx response or timeout), Omni Sign automatically retries with exponential backoff:
