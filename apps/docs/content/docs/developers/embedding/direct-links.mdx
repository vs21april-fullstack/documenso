...
When someone uses a direct link, Omni Sign creates a new document from the template with that person as the signer.
...
| Account   | Signer must be logged into a Omni Sign account |
...
Enterprise plans support white-label embedding with the "Powered by Omni Sign" badge removed. [Contact sales](mailto:support@documenso.com) for details.
